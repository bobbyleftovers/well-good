...
class Fetch_Data():
  ...
  ## Folders
  conversion_root = './private/python/transfer_to_backend_tags'
  temp_path = '.temp'
  data_path = '_data'
  posts_file_json = 'data-posts.json'
  progress_file_json = 'data-progress.json'

  fetch_posts_link = f'{target_site}/wp-json/wellandgood/v1/transfer-to-backend-tags'

  temp_dir_exists = os.path.exists(f'{conversion_root}/{temp_path}')
  data_dir_exists = os.path.exists(f'{conversion_root}/{data_path}')


  def __init__(self, tag=None, update_data=True):
    self.update_data = update_data
    self.tag = tag
    self.url_args = f'/?tag={self.tag}'
    ...
    if json_data:
      for n in json_data:
        self.temp_post_data.append({
          "id": n['id'],
          "target_backend_tag": n['target_backend_tag']
        })
