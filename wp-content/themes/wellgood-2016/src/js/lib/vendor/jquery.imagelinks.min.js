...
(function($) {
	...
	*/
	...
	ImageLinks.prototype = {
		...
		applyImage: function() {
			var imageSrc = this.container.attr("src");
        ...
        hotSpot.title = this.getPopoverTitle(hotSpot);
				hotSpot.$el.attr("id", hotSpot.id);
				hotSpot.$el.attr("data-vars-event", "image link expand");
				hotSpot.$el.attr("data-vars-info", hotSpot.title);
		...
		createPopover: function(hotSpot) {
            ...
            close_button = hotSpot.$popover.find(".imgl-close");

        hotSpot.$popover.attr("id", popoverId);

        close_button.title = this.getPopoverTitle(hotSpot);
        close_button.attr("data-vars-event", "image link close");
        close_button.attr("data-vars-info", close_button.title);
      ...
      this.createPopover(hotSpot);
		...
		getPopoverTitle: function (hotSpot) {
			if(hotSpot.popoverSelector) {
        var el = $(hotSpot.popoverSelector);
				return el.find('h3.imagelinks-meta__title').contents().text();
			}
			return "";
		},
        ...
        $('body').removeClass('js-popover-open'); // W+G
      ...
      // W+G customization: call showPopover *before* stopping propagation
			this.showPopover(hotSpot);
		...
		onPopoverHide: function(hotSpot, e) {
			...
			if(hotSpot.$popover.has(e.target).length === 0 || $(e.target).hasClass("imgl-close")) {
				if($(e.target).hasClass("imgl-close")) {
					// e.stopImmediatePropagation(); // W+G comment out
				}
