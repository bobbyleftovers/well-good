...
if( ! class_exists('WG_Varnish_Async') ) :
 ...
 * License: MIT
 */
abstract class WG_Varnish_Async {
		/**
		...
		public function __construct( $auth_level = self::BOTH ) {
			...
			if ( $auth_level & self::LOGGED_IN ) {
				add_action( "admin_post_wg_varnish_async_$this->action", array( $this, 'handle_postback' ) );
			}
			if ( $auth_level & self::LOGGED_OUT ) {
				add_action( "admin_post_nopriv_wg_varnish_async_$this->action", array( $this, 'handle_postback' ) );
			}
		}
		...
		public function remove_hooks(){
			...
			remove_action( $action, array( $this, 'launch' ) );
			remove_action( "admin_post_wg_varnish_async_$action", array( $this, 'handle_postback' ) );
			remove_action( "admin_post_nopriv_wg_varnish_async_$action", array( $this, 'handle_postback' ) );
		}
		...
		public function launch() {
			...
			$data['action'] = "wg_varnish_async_$this->action";
		...
		protected function get_nonce_action() {
			...
			$action = "wg_varnish_async_$action";
			return $action;
		}
		/**
		 ...
		 *
		 * The action should be constructed as "wg_varnish_async_task_$this->action"
		 */
